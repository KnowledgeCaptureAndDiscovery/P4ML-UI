...
def get_random_constraints(labels, numML, numCL):
    ...
    for i in xrange(c):
        ...
        c1 = np.random.randint(len(thisclass), size=numML)
        c2 = np.random.randint(len(thisclass), size=numML)
        
        # ensure no instance is constrained with itself and all constraints are unique
        for j in xrange(numML):
            while (c1[j] == c2[j] or (thisclass[c1[j]], thisclass[c2[j]]) in conset):
                c1[j] = np.random.randint(len(thisclass))
                c2[j] = np.random.randint(len(thisclass))
        ...
        c1 = np.random.randint(len(thisclass), size=numCL)
        c2 = np.random.randint(len(otherclass), size=numCL)
        # ensure uniqueness
        for j in xrange(numCL):
            while ((thisclass[c1[j]], otherclass[c2[j]]) in conset):
                c1[j] = np.random.randint(len(thisclass))
                c2[j] = np.random.randint(len(otherclass))
            # if valid, add constraint to constraint set
