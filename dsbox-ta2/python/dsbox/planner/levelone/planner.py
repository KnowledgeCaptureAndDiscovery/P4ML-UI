...
from dsbox.planner.levelone.primitives import Primitives, Category, DSBoxPrimitives, D3mPrimitives, Primitive, D3mPrimitive
...
class LevelOnePlanner(object):
    ...
    def find_primitives_by_hierarchy_new(self, dim_name, hierarchy, models, level=2):
        """Returns one particular primitive per node in the hierarchy"""
        if level == 1:
            nodes = hierarchy.get_nodes_by_level(1)
            primitives_by_node = []
            for l1_node in nodes:
            ...
            # print(models)
            new_nodes = []
            all_primitives = hierarchy.get_primitives()
            # print(all_primitives)
            for model in models:
                # print(model)
                for sub in all_primitives:
                    for elem in sub:
                        if elem.name == model:
                            nodes = hierarchy.get_node_by_primitive(elem)
                            new_nodes.append(elem)
            primitives_by_node = new_nodes
            # print(primitives_by_node)
        result = []
        for primitives in primitives_by_node:
    ...
    def generate_pipelines_with_hierarchy_new(self, models, level=2):
        """Generate singleton pipeline using tag hierarchy"""
        if self.task_type == TaskType.CLASSIFICATION:
            learning_type = TaskType.CLASSIFICATION.value  # 'classification'
            hierarchy = self.primitives.hierarchies[Category.CLASSIFICATION]
        ...
        # print(models)
        primitives = self.find_primitives_by_hierarchy_new(
            learning_type, hierarchy, models, level)
    ...
    def fill_feature_by_particular_method(self, pipeline, feature_extraction, num_pipelines=5):
        dimension_name = 'FeatureExtraction'
        current_primitive = pipeline.get_primitive(dimension_name)
        # print(current_primitive)
        pipeline_primitives = [primitive for primitive in pipeline.get_primitives()
                               if not primitive==current_primitive]
        # print(pipeline_primitives)
        hierarchy = self.primitives.hierarchies[Category.FEATURE]
        all_primitives = hierarchy.get_primitives_as_list()
        # print(all_primitives)
        ...
        primitives = []
        for primitive in all_primitives:
            for name in feature_extraction:
